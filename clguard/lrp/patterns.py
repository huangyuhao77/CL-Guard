...
from .sequential import Bottleneck, BasicBlock, PreActBlock, PreActBottleneck
...
def _prod(module, x, y, mask):
    ...
    if isinstance(module, torch.nn.Linear):
        ...
        W_fn = lambda w: w.t()  # only for linear layers 转置
        ...
        W = module.weight.view(module.out_channels, -1)  # 取出内核的weight
    ...
    # 为了计算某些统计量，例如输入的平均值，通过权重加权的输入的平均值，以及目标的全局平均值。
...
def _fit_pattern(model, train_loader, max_iter, device, mask_fn=lambda y: torch.ones_like(y)):
    ...
    first = True
    for b, (x, _, *other_info) in enumerate(tqdm(train_loader)):
        x = x.to(device)

        i = 0 # 用于记录第几层
        for m in model:
            # For Bottleneck——resnet
            ...
            if isinstance(m, PreActBlock):
                ...
                if m.shortcut is not None:
                    identity = m.shortcut[0](x)
                    mask = mask_fn(identity).float().to(device)
                    if m.shortcut[0].bias is not None:
                        y_wo_bias = y - m.shortcut[0].bias.view(-1, 1, 1)
                    else:
                        y_wo_bias = y.clone()
                    cnt_, cnt_all_, x_, y_, xy_, w, w_fn = _prod(m.shortcut[0], x, y_wo_bias, mask)
                    if first:
                        stats_x[i].append(RunningMean(x_.shape, device))
                        stats_y[i].append(RunningMean(y_.shape, device))  # Use all y
                        stats_xy[i].append(RunningMean(xy_.shape, device))
